...
const {
  generateManufacturingPaymentReceipt,
  generateAdvancePaymentReceipt,
  ...
  async (req, res) => {
    try {
      ...
      if (!order) {
        return res
          .status(404)
          .json({ message: "Manufacturing order not found" });
      }
      res.json(order);
    } catch (error) {
      res.status(400).json({ message: error.message });
...
// Update the manufacturing payment receipt route
router.post("/payments/receipt", protect, async (req, res) => {
  ...
  try {
    ...
    const receiptHtml = await generateManufacturingPaymentReceipt(payment, settings);
    res.json({ receiptHtml });
  } catch (error) {
    console.error("Error generating receipt:", error);
    res.status(500).json({ message: "Error generating receipt" });
